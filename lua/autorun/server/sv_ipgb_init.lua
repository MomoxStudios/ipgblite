...
if file.Exists("ipgb/sv_logger.lua", "LUA") then
    include("ipgb/sv_logger.lua")
else
    print("[IPGB] Logger Module is not installed. Available on Workshop")
end

IPGB.LogInstalled = IPGB.LogInstalled or false

if IPGB.LogInstalled then
    print("[IPGB] Logger Module installed and activated.")
end
...
function IPGB.Core.CheckIP(steamID64, ipAddress, svPassword, clPassword, name)
    ...
    http.Fetch(IPGB.Util.FormatIPServer(IPGB.Config["IPServer"], clientIp), function(body, len, headers, code)
        ...
        if content ~= nil then
            ...
            --badip check
            if not table.HasValue(IPGB.Whitelist["_steamID"], util.SteamIDFrom64(steamID64)) and not table.HasValue(IPGB.Whitelist["_ip"], clientIp) then
                if cc == "ZZ" and IPGB.Config["KickBadIP"] == true then
                    if (IPGB.LogInstalled) then
                        IPGB.Util.LogPlayerKickedIP(name, steamID64, clientIp)
                    end
                ...
                if IPGB.Config["ReverseWhitelist"] then
                    kickPlayer = table.HasValue(IPGB.Whitelist["_countryCode"], cc)
                end

                if not IPGB.Config["ReverseWhitelist"] then
                    kickPlayer = not table.HasValue(IPGB.Whitelist["_countryCode"], cc)
                end

                if kickPlayer then
                    if (IPGB.LogInstalled) then
                        IPGB.Util.LogPlayerKickedCC(name, steamID64, cc)
                    end
            ...
            if (IPGB.LogInstalled) then
                IPGB.Util.LogError("FATAL ERROR: IP-Api sent invalid data. Contact Developer! Source: " .. string.Replace("http://ip-api.com/json/$IP", "$IP", clientIp))
            end
        end
    end, 

    function(error)
        if error == "invalid url" and IPGB.Config["KickBadIP"] == true and not table.HasValue(IPGB.Whitelist["_ip"], clientIp) then
            if (IPGB.LogInstalled) then
                IPGB.Util.LogPlayerKickedIP(name, steamID64, clientIp)
            end
        ...
        elseif IPGB.Config["KickBadIP"] == false then
            ...
            if (IPGB.LogInstalled) then
                IPGB.Util.LogError("ERROR: Player '" .. name .. "' joined with an invalid ip and can't be checked. Refer to config for a solution.")
            end
        else
            ...
            if (IPGB.LogInstalled) then
                IPGB.Util.LogError("FATAL ERROR: Connection to IP-Api failed: " .. error .. ". Check if there are any local (bad) IPs in the IP Whitelist!")
            end
        end
    end)
end
...
timer.Create("IPGB.Util.UpdateCheck", 14400, 0, IPGB.Util.UpdateCheck) --update Check